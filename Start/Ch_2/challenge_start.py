...
def string_combiner(*args, unique=False):
    ...
    Returns a string that concatenates the args (strings and ints)
    Parameters:
    args: one or more strings and ints. Other types are ignored.
    unique: Boolean, if True, only include 1 instance of each character
    """
    result = ""
    for arg in args:
    ...
    # If unique is true, convert to a set (to make characters unique) and then a string
    if unique:
